...
**M4 закрыт; идёт M5 (сад с FSRS) — Ф0 и Ф1 готовы, сборка и тесты зелёные.** Учебное ядро работает
end-to-end (граф языка в Neo4j × «сад» в Postgres × REST): word-модель, блоки топ-слов, точные темы.
**M5** — дерево знаний как лицо сада: **Ф0** — снапшот `GET /learning/tree` (словарь+ветки+активность)
+ **журнал занятий** (фундамент под погоду/сезоны/стрики); **Ф1** — прогресс грамматики → высота
(мастерство на правилах, 2 кнопки; `grammar` в снапшоте). Следующая — **Ф2 (ядро FSRS)**. Контракт —
**3.3.0**. Полный план вех — в **[M5-PLAN.md](M5-PLAN.md)**.
