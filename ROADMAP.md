...
- **M4 — service-learning (модулит)** 🔧 (в работе: M4.1–M4.4 ✅, M4.5 контракт+фасад+REST в процессе) — учебное ядро: канонический граф языка (Neo4j) + тонкий персональный оверлей «сад» (Postgres). Первый бандед-контекст модулита; garden(полный)/tutor дорастают модулями/вехами.
