...
- **M3 — service-user** ✅ — профиль пользователя + событийная модель на Kafka (Design B). Полная сборка + CI зелёные.
  ...
  **M3 закрыт** — полная сборка `mvn -fae install` и per-module CI зелёные.
  - ✅ **follow-up: загрузка аватара (bytea)** — сделано, проверено end-to-end (загрузка + отдача картинки в браузере): контракт 2.1.0 (`GET /users/{userId}/avatar`, binary); `AvatarStoragePort` (out-port — миграция на MinIO/S3 = новый адаптер) + таблица `avatars` (Postgres **bytea**); `uploadAvatar` реальный (валидация png/jpg/webp, лимит 2MB), публичная отдача `GET /users/{id}/avatar` (permitAll и в user, и в **gateway** — `<img>` ходит без токена); `avatarUrl` загруженного файла — **полный URL** через конфиг `app.public-url` (готов для `<img src>`, строится в маппере на ответе); заглушка 501 убрана. Тесты 100%. Гоча: gateway валидирует JWT только на не-`/auth/**` путях (settings/avatar — первое боевое применение).
  Гоча для user: `eureka.instance.hostname: localhost` (✅ в yml), `/error` в permitAll (✅), id при `@GeneratedValue` руками не ставить.
- **M4 — service-learning (модулит)** ✅ **ЗАКРЫТ** (сборка + тесты зелёные) — учебное ядро: канонический граф языка (Neo4j) + тонкий персональный оверлей «сад» (Postgres). Первый бандед-контекст модулита; garden(полный)/tutor дорастают модулями/вехами.
  ...
  **Сделано:** ✅ M4.1 — скелет модулита `service-learning` (3 модуля, порт 8083), Neo4j+Postgres со скоупом репозиториев (`@EnableNeo4jRepositories`/`@EnableJpaRepositories`), resource-server, docker-compose +Neo4j, gateway-роут `/api/v1/learning/**`; сборка зелёная.
  ...
  ✅ M4.4 — garden-контекст: домен `Mastery`/`MasteryStatus`(KNOWN/LEARNING/UNKNOWN) + порты + `MasteryUseCaseImpl` (setStatus/statuses/myMastery); JPA-персистентность (`MasteryEntity` со составным ключом `@IdClass`, `MasteryRepositoryAdapter`, `findByUserIdAndLexemeIdIn` для раскраски) + Liquibase-таблица `mastery`; `lexeme_id` = id узла Neo4j строкой без FK (два графа); `GardenUseCaseConfig` + юнит-тест. Гоча: `mockito-junit-jupiter` — отдельный артефакт от `mockito-core` (в чистом logic-модуле нужно добавлять явно). Зелёное.
  ✅ M4.5 — REST-слой + garden-view: контракт `learning-api.yaml` (OAS 2.2.0, lockstep; +фикс описания `avatarUrl`); фасад `LearningQueryUseCase` (джойн канона Neo4j × статуса Postgres по id → `LexemeCard`/`TopicView` с раскраской); learning-api (`LearningController` реализует `LearningApi`, `LearningApiMapper` домен→DTO, `CurrentUser`, exception-handler) + learning-app (`SecurityContextCurrentUser`, `LearningUseCaseConfig`); генерация contract-first как в user. Тесты: фасад/контроллер/маппер/handler/current-user. Зелёное.
  ✅ M4.6 — тесты **100%** (юниты logic/api + интеграция `LearningFlowTest` на Testcontainers Neo4j+Postgres в app) + CI. Гоча: два Spring Data в модулите → оба tx-менеджера объявлены явно в `TransactionConfig` (JPA `@Primary`, Neo4j с `DatabaseSelectionProvider`), иначе NPE на первом запросе к графу; Neo4j-база переехала на дефолт `neo4j` (проще, убирает гочу «не та база»).
  ✅ M4.7 — **переработка импорта** (данные стали годными): дедуп по лемме (словоформы `went/goes` не отдельные слова, частота свёрнута в лемму); второй источник переводов **WikDict** (`--dict` TSV, fallback на лемму без POS + чистка wiki-разметки `[[...]]`); темы из kaikki-категорий (`--topics seed/topics.tsv`, курируемый allow-list + primary-sense против шума); грамматика (`--grammar seed/grammar.tsv` по British Council/EAQUALS Core Inventory + маркер-слова + авто-`ILLUSTRATES` из неправильных форм); **IPA** (`sounds[].ipa`); отсев alt-форм/мультислов/аббревиатур. Темы и грамматику теперь **строит сам тул** из `seed/*.tsv` — ручной `cypher-shell` убран, `topics-grammar.cypher` удалён.
  ✅ M4.8 — **word-модель** (контракт **3.0.0**, ломающий): единица обучения = **слово (лемма)** `en:go`, части речи склеены в `variants[]`; мастерство **по лемме**; DTO `WordCard`/`WordVariant`/`WordRef`/`WordLeaf`/`WordPage`; `GET /learning/words/{id}` (было `/lexemes`); агрегация POS-узлов лемм в адаптере через Neo4jClient. Удалены `RelationType`, `Lexeme`/`LexemeRef`/`LexemeCard`/`GardenLeaf`.
  ✅ M4.9 — **блоки топ-слов** (контракт **3.1.0**, additive): `GET /learning/bands` — уровни по частоте (топ-100/…/5001–10000, эксклюзивные) с прогрессом `total/known/learning`; `/words?band=` — слова блока с `WordLeaf.topics` (разбивка по категориям); `/study` — очередь «Учить». Мастерство на фронте — **2 кнопки Знаю/Учить** (KNOWN/LEARNING; UNKNOWN = не отмечено). Частотные банды = первичная навигация, категории — вторичная разбивка внутри блока (растворяет проблему шумных авто-тем).
  ✅ M4.10 — **качество данных + точные темы** (тул, контракт не тронут): стоп-лист служебных слов (`--stopwords`, мимо графа), ранг по самой лемме (не свёртка по формам — убрана инфляция `ha←has`), плотное переранжирование по лемме (блоки ровные 100/400/500/2000/2000/864); темы — три слоя: курируемый сид `слово→ветка` (`--word-topics`, в приоритете) + вычищенный `topics.tsv` + фолбэк по этимологии-1. Промахи категорий убраны.
...
- **M5 — сад с FSRS** 🚧 **В РАБОТЕ** — план в **[M5-PLAN.md](M5-PLAN.md)** (дерево = лицо сада). FSRS-retrievability вместо статуса: «увядание» листьев, цвет = вероятность вспомнить, планировщик повторений; прогресс грамматики → высота; журнал активности → стрики/погода/сезоны. Фазы: **Ф0** снапшот `GET /learning/tree` + журнал активности (контракт 3.2.0) → **Ф1** грамматика-прогресс (3.3.0) → **Ф2** ядро FSRS (4.0.0). Решения: полный FSRS-5, 2 кнопки повторения (Забыл/Помню), одна `review_state`+`nodeKind`.
...
- **Модулит с двумя Spring Data — нужны ДВА tx-менеджера явно.** И JPA-, и Neo4j-автоконфиг объявляют свой через `@ConditionalOnMissingBean`, поэтому создаётся только один (JPA успевает первым). А SDN ищет бин **именно типа** `Neo4jTransactionManager` — без него `Neo4jTemplate` остаётся без transaction-template и первый же запрос к графу падает с `NullPointerException` (в конструкторе `DefaultExecutableQuery`). Решение — `TransactionConfig` в learning-app: оба менеджера явно, JPA — `@Primary`, Neo4j — с `DatabaseSelectionProvider` (иначе пойдёт не в ту базу). Не всплыло в тестах, потому что Neo4j-тест был назван `*IT` и Surefire его не гонял (Failsafe не настроен).
